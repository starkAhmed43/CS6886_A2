...
def test_fake_quantize_roundtrip_tight():
    """8-bit fake quantization stays within one step of the original values."""
    torch.manual_seed(0)
    x = torch.randn(10000)
...
def test_per_channel_weight_qparams_shape():
    """Per-channel weight qparams broadcast against a conv weight [O, I, H, W]."""
...
def test_get_qmin_qmax():
    """Integer grid bounds are correct for signed and unsigned 8-bit."""
    assert get_qmin_qmax(8, False) == (0, 255)
    assert get_qmin_qmax(8, True) == (-127, 127)


def test_model_size_report_linear_8bit():
    """An all-8-bit quantized linear reports roughly 4x model compression."""
    ...
    report = model_size_report(
        net, [ql], weight_bits=8, activation_bits=8, per_channel_weights=True
    )
